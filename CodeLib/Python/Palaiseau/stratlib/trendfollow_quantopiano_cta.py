...
class TrendFollow(strategy.BacktestingStrategy):
    def __init__(self, feed, instrument, lookback_periods, profit_take_std, slope_min_positive, slope_min_negative = None,
                 maxdrawdown = 0.06, maxdrawdown_period = 150, info_show=True, instrumentMainFutContract = None, instrumentSubFutContract = None,futContractClearingDate=None):
    ...
    def onBars(self, bars):
        ...
        if self.__longPos is not None:
             if self.exitLongSignal(slope,delta_neg_one) and not ( self.__useFutForClearing and  bars[self.__instrument].getDateTime() in self.__futContractClearingDate):
                 self.__longPos.exitMarket()
                 if self.__info_show:
                    self.info("LONG close at $%.2f at %s" % (bars[self.__instrument].getPrice(),bars[self.__instrument].getDateTime()))
        # close short position
        elif self.__shortPos is not None:
             if self.exitShortSignal(slope,delta_neg_one) and not ( self.__useFutForClearing and  bars[self.__instrument].getDateTime() in self.__futContractClearingDate):
                 self.__shortPos.exitMarket()
                 if self.__info_show:
                    self.info("SHORT close at $%.2f at %s" % (bars[self.__instrument].getPrice(),bars[self.__instrument].getDateTime()))
        # open long position

        if  (self.__longPos is  None) and (self.__shortPos is None) or self.__shortYicang or self.__longYicang :
             if self.__longYicang:
                 shares = int(self.getBroker().getEquity() *0.5 / bars[self.__instrumentSubFutContract].getPrice())
                 self.__longPos = self.enterLong(self.__instrumentSubFutContract, shares)
                 self.__longYicang = False
                 if self.__info_show:
                      self.info("LONG open at $%.2f at %s" % (bars[self.__instrument].getPrice(),bars[self.__instrument].getDateTime() ))
             elif self.__shortYicang:
                 shares = int(self.getBroker().getEquity()*0.5  / bars[self.__instrumentSubFutContract].getPrice())
                 self.__shortPos = self.enterShort(self.__instrumentSubFutContract, shares)
                 self.__shortYicang =False
                 if self.__info_show:
                ...
                if self.enterLongSignal(delta_neg_one,delta_neg_two,slope,dd_long):
                   shares = int(self.getBroker().getEquity() *0.5 / bars[self.__code].getPrice()) # 5成仓位
                   self.__longPos = self.enterLong(self.__code, shares)
                   if self.__info_show:
                      self.info("LONG open at $%.2f at %s" % (bars[self.__instrument].getPrice(),bars[self.__instrument].getDateTime() ))
        # open short position2
                elif self.enterShortSignal( delta_neg_one,delta_neg_two,slope,dd_short):
                    shares = int(self.getBroker().getEquity()*0.5  / bars[self.__code].getPrice()) # 5成仓位
                    self.__shortPos = self.enterShort(self.__code, shares)
                    if self.__info_show:
                       self.info("SHORT open at $%.2f at %s" % (bars[self.__instrument].getPrice(),bars[self.__instrument].getDateTime()))
        if self.__useFutForClearing and  bars[self.__instrument].getDateTime() in self.__futContractClearingDate:
                if self.__longPos is not None:
                   self.__longPos.exitMarket()
                   self.__longYicang = True
                   if self.__info_show:
                    self.info("LONG close at $%.2f at %s" % (bars[self.__instrument].getPrice(),bars[self.__instrument].getDateTime()))

                elif self.__shortPos is not None:
                   self.__shortPos.exitMarket()
                   self.__shortYicang = True
                   if self.__info_show:
                    self.info("SHORT close at $%.2f at %s" % (bars[self.__instrument].getPrice(),bars[self.__instrument].getDateTime()))
...
if __name__ == "__main__":
    ...
    if source == 'tushare':
        ...
        barfeed.addBarsFromCSV('ic', "..//histdata//min//ic.csv")
        barfeed.addBarsFromCSV('ic00', "..//histdata//min//ic00.csv")
        barfeed.addBarsFromCSV('ic01',"..//histdata//min//ic01.csv")
    ############################################Run the strategy#########################
    strat1 = strat(barfeed, instrument, *paras,instrumentMainFutContract='ic00',instrumentSubFutContract='ic01',futContractClearingDate=futClearingDate)
    ...
    performanceAnalyzer.getReturnsTearSheet(live_start_date = '2016-02-01')
