...
class BacktestingStrategy(BaseStrategy):
    ...
    def __init__(self, barFeed, cash_or_brk=1000000):
        ...
        self.__info = pd.DataFrame(columns={'date','id','action','instrument','quantity','price'})   #交易记录信息
        self.__info_matrix = []
    ...
    def getInfo(self):
        ...
        return pd.DataFrame({'date':_matrix[:,0],'id':_matrix[:,1],'action':_matrix[:,2],'instrument':_matrix[:,3],'quantity':_matrix[:,4],'price':_matrix[:,5]})

    def onEnterOk(self, position):
        self.addInfo(position.getEntryOrder())

    def onExitOk(self, position):
        self.addInfo(position.getExitOrder())
