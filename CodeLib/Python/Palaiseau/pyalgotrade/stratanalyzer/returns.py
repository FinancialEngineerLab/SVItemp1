...
class Returns(stratanalyzer.StrategyAnalyzer):
    ...
    def getAnnualReturn(self):
        firstDateTime = self.__cumReturns.getDateTimes()[0]
        lastDateTime = self.__cumReturns.getDateTimes()[-1]
        yearsTraded = date_count.years_traded(firstDateTime, lastDateTime)
        self.__annualReturn = self.__cumReturns.getValueAbsolute(self.__cumReturns.__len__() - 1) / yearsTraded
