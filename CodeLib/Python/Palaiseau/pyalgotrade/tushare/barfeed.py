...
class WindPollingThread(threading.Thread):
    # Not using xignite polling thread is because two underscores functions can't be override, e.g. __wait()

    WIND_INQUERY_PERIOD = 6 # tushare read period, default is 3s

    def __init__(self, identifiers):
        super(WindPollingThread, self).__init__()
        self._identifiers = identifiers
        self._tickDSDict = {}
        self._last_quotation_time = {}
        WindPy.w.start()
    ...
    def __wait(self):
        ...
        while not self.__stopped and utcnow() < nextCall:
            ...
            if time_diff < WindPollingThread.WIND_INQUERY_PERIOD:
                time.sleep(WindPollingThread.WIND_INQUERY_PERIOD - time_diff)
    ...
    def get_tushare_tick_data(self):
        try:
            ...
            for index, identifier in enumerate(self._identifiers):
                tick_info = WindPy.w.wsq(identifier, "rt_last,rt_last_vol,rt_last_amt,rt_time")
                #print tick_info.Data[0][0]
                if True:
                    # tushare use unicode type, another way is convert it to int/float here. refer to build_bar
                    self._tickDSDict[identifier].append(tick_info.Data[0][0], tick_info.Data[1][0], tick_info.Data[2][0],
                                                        unicode(str(tick_info.Data[3][0])[0:2]+':'+str(tick_info.Data[3][0])[2:4]+':'+str(tick_info.Data[3][0])[4:6],'GBK'))
        except Exception, e:
            logger.error("Tushare polling exception", exc_info=e)
...
class WindBarFeedThread(WindPollingThread):
    # Events
    ON_BARS = 1

    def __init__(self, queue, identifiers, frequency):
        super(WindBarFeedThread, self).__init__(identifiers)
        self.__queue = queue
        self.__frequency = frequency
        self.__updateNextBarClose()
...
class TuShareLiveFeed(barfeed.BaseBarFeed):
    ...
    def __init__(self, identifiers, frequency,path, maxLen=dataseries.DEFAULT_MAX_LEN, replayDays=-1,use_wind=False):
        ...
        if use_wind:
           self.__thread = WindBarFeedThread(self.__queue, identifiers, frequency)
        else:
        ...
        for identifier in self.__identifiers:
            try:
                ...
                if df is not None:
                    ...
                    today_bars[identifier] =[]
                    for line in open(self.__path):                        #读取前一天数据
                        date_time,open_,high,low,close = line.split(",")
                        date_time = date_time[0:len(date_time)-10]
                        date_time = date_time.replace('-','/')
                        date_time = datetime.datetime.strptime(date_time,'%Y/%m/%d %H:%M')
                        date_time = date_time.replace(tzinfo=pytz.timezone('Asia/Shanghai'))
                        open_ = float(open_)
                        high = float(high)
                        low = float(low)
                        close = float(close)
                        today_bars[identifier].append(bar.BasicBar(date_time, open_, high, low,
                                         close, 10, 0, 300, 100000000))
            except Exception, e:
                logger.error(e)
        #today_bars[identifier].reverse()
        ...
        for day in get_trading_days(now, replay_days):
            ...
            for identifier in self.__identifiers:
                ...
                if df is not None:
                    ...
                    bars_dict[identifier] =[]
                    for line in open("5min_data_yesterday.csv"):                  #读取前天数据
                        date_time,open_,high,low,close = line.split(",")
                        date_time = datetime.datetime.strptime(date_time,'%Y/%m/%d %H:%M')
                        date_time = date_time.replace(tzinfo=pytz.timezone('Asia/Shanghai'))
                        open_ = float(open_)
                        ...
                        low = float(low)
                        close = float(close)
                        bars_dict[identifier].append(bar.BasicBar(date_time, open_, high, low,
                                          close, 10, 0, 300, 100000000))

            #bars_dict[identifier].reverse()
...
if __name__ == '__main__':
    df =ts.get_realtime_quotes('zh500')
    liveFeed = TuShareLiveFeed(['zh500'],300, 100,1 )
    liveFeed.start()
    ...
    while not liveFeed.eof():
        ...
        if bars is not None:
            print bars['zh500'].getHigh(), bars['zh500'].getDateTime()
             # test/
