...
class SlopeEventWindow(technical.EventWindow):
    ...
    def getSlopeAndConstant(self):
        a, b = None, None
        if self.windowFull():
            y = self.getValues()
            a, b = lsreg(self.__x, y)
        return a,b
...
class Slope(technical.EventBasedFilter):
    ...
    def __init__(self, dataSeries, period, maxLen=dataseries.DEFAULT_MAX_LEN):
        ...
        self.__constant = dataseries.SequenceDataSeries(maxLen)
        self.__slope = dataseries.SequenceDataSeries(maxLen)
        dataSeries.getNewValueEvent().subscribe(self.__onNewValue)

    def __onNewValue(self, dataseries, dateTime, value):
        a, b = self.getEventWindow().getSlopeAndConstant()
        self.__slope.append(a)
        self.__constant.append(b)
    ...
    def getRegValue(self, x_value):
        return self.__slope[-1] * x_value + self.__constant[-1]
