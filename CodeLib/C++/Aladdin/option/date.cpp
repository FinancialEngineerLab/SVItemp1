...
//return the month of Date variable
...
//return the year of Date variable
...
Date Date::advance(const Date& date, int n, TimeUnit units) 
{
    ...
    switch (units) {
        ...
        case Months: 
			{
				...
				ALADDIN_ENSURE(y >= 1900 && y <= 2199,
							"year " << y << " out of bounds. "
							<< "It must be in [1901,2199]");

				length = monthLength(Month(m), isLeap(y));
				if (d > length)
					d = length;
        ...
        default:
			ALADDIN_FAIL("undefined time units");
    }
}
...
bool Date::isLeap(Year y)
{
	static const bool YearIsLeap[] = {
		...
		ALADDIN_ENSURE((y>=1900 && y<=2200),"Only years between 1900 - 2200 is acceptable");
...
int Date::minimumSerialNumber() 
{
    return 367;       // Jan 1st, 1901
}

int Date::maximumSerialNumber() 
{
    return 109574;    // Dec 31st, 2199
}

 void Date::checkSerialNumber(int serialNumber) 
 {
    ALADDIN_ENSURE(serialNumber >= minimumSerialNumber() &&
