...
namespace QuantLibAddin {
	...
	//aladdin
	boost::shared_ptr<QuantLib::Payoff> BARRIERKOKO_Payoff(
		const double underlyingSpot,
		const double margin,
		const double gearing1, 
		const double gearing2,
		const double lowerBarrier,
		const double upperBarrier,
		const double lowerInBarrier,
		const double upperInBarrier) {
			return boost::shared_ptr<QuantLib::Payoff> (
				new QuantLib::BarrierKOKOPayoff(underlyingSpot, margin,gearing1, gearing2, lowerBarrier, upperBarrier, lowerInBarrier, upperInBarrier));
	}
	...
	//aladdin
	boost::shared_ptr<QuantLib::PricingEngine> MCE_Engine(
		const boost::shared_ptr<QuantLib::GeneralizedBlackScholesProcess>& process, const long& timeSteps, const bool& brownianBridge,
		const bool& Antithetic, const long& Samples, const double& absoluteTolerance, const long& maxSamples, const long& seed) {
			return boost::shared_ptr<QuantLib::PricingEngine> (
				new QuantLib::MCEuropeanEngine<>(process, timeSteps,QL_NULL_INTEGER,brownianBridge,Antithetic,Samples,absoluteTolerance,maxSamples,seed));
	}


	boost::shared_ptr<QuantLib::PricingEngine> MCKOKO_Engine(
		const boost::shared_ptr<QuantLib::GeneralizedBlackScholesProcess>& process, const long& timeSteps, const bool& brownianBridge,
		const bool& antitheticVariate, const long& Samples, const double& absoluteTolerance, const long& maxSamples, const long& seed)
	{
		return boost::shared_ptr<QuantLib::PricingEngine> (
			new QuantLib::MCBarrierKOKOEngine<>(process, timeSteps,QL_NULL_INTEGER,brownianBridge,antitheticVariate,Samples,absoluteTolerance,maxSamples,seed));
	}
