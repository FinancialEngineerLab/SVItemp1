...
namespace ObjectHandler {
	...
	// A function to construct a PricingEngine - Monte Carlo Variants -aladdin
	typedef boost::shared_ptr<QuantLib::PricingEngine>(*PricingEngineConstructor3)(
		const boost::shared_ptr<QuantLib::GeneralizedBlackScholesProcess>&, const long&, const bool&, const bool&, const long&,const double&,const long&, const long&);
	...
	// Create class template specialization for PricingEngine - with Monte Carlo Variants -aladdin
	template<>
	class Create<PricingEngineConstructor3> :
		private RegistryManager<PricingEngineConstructor3, EnumClassRegistry> {
	public:
		PricingEngineConstructor3 operator()(const std::string& engineID) {
			return reinterpret_cast<PricingEngineConstructor3>(getType(engineID));
		}
		using RegistryManager<PricingEngineConstructor3, EnumClassRegistry>::registerType;
	};
	...
	//A wrapper for Create - with Monte Carlo Variants - aladdin
	inline boost::shared_ptr<QuantLib::PricingEngine> createPricingEngine(
		const std::string& engineID,
		const boost::shared_ptr<QuantLib::GeneralizedBlackScholesProcess>& process,
		const long& timeSteps,
		...
		const long& seed) {
			...
			return Create<PricingEngineConstructor3>()(engineID)(process, timeSteps, brownianBridge, antitheticVariate, Samples, absoluteTolerance, maxSamples, seed);
	}
 }
