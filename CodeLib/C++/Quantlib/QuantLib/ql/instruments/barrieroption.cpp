...
namespace QuantLib {
	...
	BarrierKOKOOption::BarrierKOKOOption(
		...
		const boost::shared_ptr<Exercise>& exercise)
		: OneAssetOption(payoff, exercise),
		barrierType_(barrierType), barrierUp_(barrierUp),barrierDown_(barrierDown){}

	void BarrierKOKOOption::setupArguments(PricingEngine::arguments* args) const {     

		OneAssetOption::setupArguments(args);
		
		BarrierKOKOOption::arguments* moreArgs = 
			dynamic_cast<BarrierKOKOOption::arguments*>(args);
		//QL_REQUIRE(moreArgs != 0, "wrong argument type");
		if(moreArgs) // temp workout to skip this check when using european engine
		{
			moreArgs->barrierType = barrierType_;
			moreArgs->barrierUp = barrierUp_;
			moreArgs->barrierDown = barrierDown_;
		}
	}


	Volatility BarrierKOKOOption::impliedVolatility(
		Real targetValue,
		const boost::shared_ptr<GeneralizedBlackScholesProcess>& process,
		Real accuracy,
		Size maxEvaluations,
	...
	BarrierKOKOOption::arguments::arguments()
		: barrierType(Barrier::Type(-1)), barrierUp(Null<Real>()), barrierDown(Null<Real>()) {}

	void BarrierKOKOOption::arguments::validate() const {
		OneAssetOption::arguments::validate();
		...
		QL_REQUIRE(barrierUp != Null<Real>(), "no upper barrier given");
		QL_REQUIRE(barrierDown != Null<Real>(), "no lower barrier given");
	}

	bool BarrierKOKOOption::engine::triggered(Real underlying) const {
		switch (arguments_.barrierType) {
		case Barrier::DownIn:
		case Barrier::DownOut:
			return underlying < arguments_.barrierUp;
		case Barrier::UpIn:
		case Barrier::UpOut:
			return underlying > arguments_.barrierDown;
		case Barrier::KOKO:
			return  underlying < arguments_.barrierDown || underlying > arguments_.barrierUp;
		default:
			QL_FAIL("unknown type");
		}
	}
