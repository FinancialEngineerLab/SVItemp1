    ...
    class GeneralStatistics {
      public:
        ...
        */
		...
		//! aladdin
		Real maxDrawdown(bool bUseRatio=0) const;
		void getSampleValues() const;  // returns a vector of samples values, removing weights
		Disposable<Matrix> frequency(int iBreaks = 0) const;  //返回区间和频数

        //@}
      private:
        mutable std::vector<std::pair<Real,Real> > samples_;  //  <value,weight>
		mutable std::vector<Real> sampleValues_;  //  <value>
