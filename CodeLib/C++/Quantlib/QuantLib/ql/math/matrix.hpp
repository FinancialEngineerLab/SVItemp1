...
namespace QuantLib {
    ...
    */
	...
	std::vector<std::vector<Real> > to2DVector(const Matrix& m); 
	...
	// aladdin
	inline std::vector<std::vector<Real> > to2DVector(const Matrix& m) {
		std::vector<std::vector<Real> > ret(m.rows(),m.columns());
