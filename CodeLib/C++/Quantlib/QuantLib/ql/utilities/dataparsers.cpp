...
namespace QuantLib {
    ...
    Period PeriodParser::parse(const std::string& str) {
        ...
        while (reducedStringDim>0) {
            iPos = reducedString.find_first_of("DdWwMmYyTt"); // aladdin: add a "T" type string
    ...
    Period PeriodParser::parseOnePeriod(const std::string& str) {
        ...
        Size iPos = str.find_first_of("DdWwMmYyTt");//aladdin
		...
		else if (abbr == 'T') units = NonBusinessDays; //aladdin
