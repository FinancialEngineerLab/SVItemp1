...
namespace QuantLib {
	...
	Real BlackScholesFormula(Option::Type optionType,
								Real strike,
								Real spot,
								Real sigma,
								Real riskFreeRate,
								Real time)
	{
		QL_REQUIRE(strike>=0.0,
			"stdDev (" << strike << ") must be non-negative");
		QL_REQUIRE(spot>0.0,
			"discount (" << spot << ") must be positive");
		QL_REQUIRE(sigma>=0.0,
			"stdDev (" << sigma << ") must be non-negative");
		QL_REQUIRE(time>0.0,
			"discount (" << time << ") must be positive");

		if (sigma==0.0)
			return std::max((spot-strike)*optionType, Real(0.0));

		Real d1 = (std::log(spot/strike) + riskFreeRate*time)/(sigma*sqrt(time))+0.5*sigma*sqrt(time);
		Real d2 = d1 - sigma*sqrt(time);
		CumulativeNormalDistribution phi;
		Real nd1 = phi(optionType*d1);
		Real nd2 = phi(optionType*d2);
		Real result = optionType * (spot*nd1 - strike*std::exp(-riskFreeRate*time)*nd2);
	...
	Real BlackScholesDelta(Option::Type optionType,
							Real strike,
							Real spot,
							Real sigma,
							Real riskFreeRate,
							Real time)
	{
		...
		CumulativeNormalDistribution phi;
		Real nd1 = phi(optionType*d1);
		Real result = optionType*nd1;
		return result;
	}
      
	Real BlackScholesTheta(Option::Type optionType,
							Real strike,
							Real spot,
							Real sigma,
							Real riskFreeRate,
							Real time)
	{
		...
		Real d2 = d1 - sigma*sqrt(time);
		CumulativeNormalDistribution phi;
		Real result = -spot*CumulativeNormalDistribution().derivative(d1)*sigma/(2*sqrt(time))-optionType*riskFreeRate*strike*exp(-riskFreeRate*time)*phi(optionType*d2);
		return result;
	}


	Real BlackScholesGamma(Option::Type optionType,
							Real strike,
							Real spot,
							Real sigma,
							Real riskFreeRate,
							Real time)
	{
		...
		Real d1 = (std::log(spot/strike) + riskFreeRate*time)/(sigma*sqrt(time))+0.5*sigma*sqrt(time);
		CumulativeNormalDistribution phi;
		Real result = CumulativeNormalDistribution().derivative(d1)/(spot*sigma*sqrt(time));
		return result;
	} 

	Real BlackScholesVega(Option::Type optionType,
							Real strike,
							Real spot,
							Real sigma,
							Real riskFreeRate,
							Real time)
	{
		...
		Real d1 = (std::log(spot/strike) + riskFreeRate*time)/(sigma*sqrt(time))+0.5*sigma*sqrt(time);
		CumulativeNormalDistribution phi;
		Real result = CumulativeNormalDistribution().derivative(d1)*spot*sqrt(time);
		return result;
	} 

	Real BlackScholesRho(Option::Type optionType,
						Real strike,
						Real spot,
						Real sigma,
						Real riskFreeRate,
						Real time)
	{
		...
		Real d2 = d1 - sigma*sqrt(time);
		CumulativeNormalDistribution phi;
		Real result = optionType*phi(optionType*d2)*strike*time*exp(-riskFreeRate*time);
		return result;
	} 

	//aladdin: implied vol computation given spot price and spot strike
	Real BalckScholesImpliedVol(Option::Type optionType,
								Real strike,
								Real spot,
								Real optionPrice,
								Real riskFreeRate,
								...
								Natural maxIterations)
	{
		Real result =  blackFormulaImpliedStdDev(optionType,
												 strike,
												 spot*exp(riskFreeRate*time),
												 optionPrice,
												 exp(-riskFreeRate*time),
												 displacement,
												 guess,
												 accuracy,
												 maxIterations);
		return result/sqrt(time); //return the annual vol 
	}
	/*
	double StevenFormulaImpliedVolatility(Option::Type optionType,
		Real strike,
		Real spot,
		Real optionPrice,
		Real freeRate,
		...
		)
	{
		...
		QL_REQUIRE(spot>0.0,
			"discount (" << spot << ") must be positive");
		QL_REQUIRE(optionPrice>=0.0,
			"stdDev (" << optionPrice << ") must be non-negative");
		QL_REQUIRE(time>0.0,
			"discount (" << time << ") must be positive");
		...
		K = strike*exp(-freeRate*time);
		eta = K/spot;
		rho = abs(eta - 1)/((optionPrice/spot)*(optionPrice/spot));
		alpha = sqrt(2*M_PI)/(1+eta)*(2*optionPrice/spot + eta - 1);
