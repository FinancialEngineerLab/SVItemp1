...
namespace QuantLib {
	...
	BarrierKOKOPathPrice::BarrierKOKOPathPrice(
		Barrier::Type barrierType,
		Real barrier_low,
		Real barrier_high,
		Real rebate,
		...
		//Option::Type type,
		//Real strike, strike is not useful for this type of option
		const std::vector<DiscountFactor>& discounts,
		const boost::shared_ptr<StochasticProcess1D>& diffProcess,
		const PseudoRandom::ursg_type& sequenceGen)
		: barrierType_(barrierType), barrier_low_(barrier_low),barrier_high_(barrier_high),
		rebate_(rebate), diffProcess_(diffProcess),
		sequenceGen_(sequenceGen),payoff_(payoff),
		discounts_(discounts) {
			
			QL_REQUIRE(barrier_low_>0.0,
				"barrier_low less/equal zero not allowed");
			QL_REQUIRE(barrier_high_>0.0,
				"barrier_high less/equal zero not allowed");
	}

	
	Real BarrierKOKOPathPrice::operator()(const Path& path) const {
		static Size null = Null<Size>();
		Size n = path.length();
		QL_REQUIRE(n>1, "the path cannot be empty");
		...
		// use no corrections for barrier: to be checked
		switch (barrierType_) {
		case Barrier::KOKO:
			isOptionActive = true;
			for (i = 1; i < n; i++) {
				asset_price = path[i];
				if (asset_price <= barrier_low_ || asset_price >=barrier_high_ ) {
					isOptionActive = false;
					if (knockNode == null)
						knockNode = i;
				}
