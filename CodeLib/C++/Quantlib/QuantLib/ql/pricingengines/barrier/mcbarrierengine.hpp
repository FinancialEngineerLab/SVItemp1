...
namespace QuantLib {
    ...
    */
	...
	//Pricing Engine for Barrier Option KOKO when using Monte Carlo Methods -Aladdin
	template <class RNG = PseudoRandom, class S = Statistics>
	class MCBarrierKOKOEngine : public BarrierKOKOOption::engine,
		public McSimulation<SingleVariate,RNG,S> {
	public:
		typedef
			typename McSimulation<SingleVariate,RNG,S>::path_generator_type
	...
	//aladdin
	class BarrierKOKOPathPrice: public PathPricer<Path>{
	public:
		BarrierKOKOPathPrice(
			Barrier::Type barrierType,
			Real barrier_low,
			Real barrier_high,
			Real rebate,
			const boost::shared_ptr<BarrierKOKOPayoff> payoff,
			const std::vector<DiscountFactor>& discounts,
			const boost::shared_ptr<StochasticProcess1D>& diffProcess,
			const PseudoRandom::ursg_type& sequenceGen);
		Real operator()(const Path& path) const;
	private:
		...
		boost::shared_ptr<StochasticProcess1D> diffProcess_;
		PseudoRandom::ursg_type sequenceGen_;
		boost::shared_ptr<BarrierKOKOPayoff> payoff_;
		std::vector<DiscountFactor> discounts_;
	...
	template <class RNG, class S>
	inline 
	MCBarrierKOKOEngine<RNG,S>::MCBarrierKOKOEngine(
		const boost::shared_ptr<GeneralizedBlackScholesProcess>& process,
		Size timeSteps,
		Size timeStepsPerYear,
		bool brownianBridge,
		...
		Real requiredTolerance,
		Size maxSamples,
		//bool isBiased, not useful to KOKO
		BigNatural seed)
		: McSimulation<SingleVariate,RNG,S>(antitheticVariate, false),
		process_(process), timeSteps_(timeSteps),
		timeStepsPerYear_(timeStepsPerYear),
	...
	template <class RNG, class S>
	inline TimeGrid MCBarrierKOKOEngine<RNG,S>::timeGrid() const {
	...
	template <class RNG, class S>
	inline
	boost::shared_ptr<typename MCBarrierKOKOEngine<RNG,S>::path_pricer_type>
	MCBarrierKOKOEngine<RNG,S>::pathPricer() const {
		boost::shared_ptr<BarrierKOKOPayoff> payoff =
			boost::dynamic_pointer_cast<BarrierKOKOPayoff>(arguments_.payoff);
		QL_REQUIRE(payoff, "non-plain payoff given");
		...
		PseudoRandom::ursg_type sequenceGen(grid.size()-1,
			PseudoRandom::urng_type(5));
		return boost::shared_ptr<
			typename MCBarrierKOKOEngine<RNG,S>::path_pricer_type>(
			new BarrierKOKOPathPrice(
			arguments_.barrierType,
			arguments_.barrierDown,
			arguments_.barrierUp,
 			payoff->margin(),  //value equivalent to arguments_.rebate,
			payoff,
			discounts,
			process_,
			sequenceGen));		
